...
def make_plots():
    # df = fix_df()
    df = pd.read_csv('data_assignment2_test_schedule.csv')
    df = df.groupby(["n","m","threads"]).agg({'time':['mean','sem'], 'flops':['mean', 'sem']}).reset_index()
    ...
    plot_speed_up(ax3, ax4, 1, 1, df, df_seq, experiment_tuples)

    # fig1.savefig("flops.png")
    # fig2.savefig("times.png")
    fig3.savefig("speed-up_heat_test.png")
    fig4.savefig("speed-up_heat_seq_test.png")
