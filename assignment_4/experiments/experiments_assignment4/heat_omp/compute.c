...
void do_compute(const struct parameters* p, struct results *r)
{
    ...
    const int M = p->M;
    int converged, compute_statistics;
    ...
    do {
        ...
        #pragma omp parallel for num_threads(nthreads) reduction(max: maxdiff)
        for (int row_offset=M; row_offset < grid_end; row_offset+=M)
        {
            // Inner cells
            int index_right_col = row_offset + M-1;
            for (int index=row_offset+1; index < index_right_col; index++)
            {
                const double old_temperature = temperature_old[index];
                const double c_c = conductivity[index - M];
                const double rest_c_c = 1 - c_c;
                // Scale old temperature at the cell

                double new_temperature = old_temperature * c_c;
                // Adjacent neighbors
                ...
                const double diff = fabs(old_temperature - new_temperature);
                if (diff > maxdiff){
                    maxdiff = diff;
                }
            }
            ...
            // Outer cells
            const int index_left_col = row_offset;
            const double old_temperature_left = temperature_old[index_left_col];
            const double old_temperature_right = temperature_old[index_right_col];

            // Left
            // Scale old temperature at the cell
            double c_c = conductivity[index_left_col - M];
            double rest_c_c = 1 - c_c;
            double new_temperature = old_temperature_left * c_c;
        ...
        if (maxdiff < threshold) converged=1;
        ...
        if (compute_statistics || converged){
            ...
            for (int index = M; index < grid_end; ++index){
                double new_temperature = temperature_new[index];
