...
__device__ __constant__ double d_c_cdir;
__device__ __constant__ double d_c_cdiag;
...
__global__ void update_temperatures(double * temperature_old, double * temperature_new, double * conductivity, const int M, const int N)
{
    ...
    __shared__ double temperatures[BLOCK_SIZE_X][BLOCK_SIZE_Y];
    ...
    if ((row < N) && (col < M))
    {
        temperatures[tidy][tidx] = temperature_old[index];
    }
    __syncthreads();

    if ((!tidx) && (tidx < BLOCK_SIZE_X) && (!tidy) && (tidy < BLOCK_SIZE_Y) && (row < N + 1) && (col < M))
    {
        ...
        new_temperature += (temperatures[tidy][tidx-1] + temperatures[tidy][tidx+1] +
                temperatures[tidy-1][tidx] + temperatures[tidy+1][tidx]) * rest_c_c * d_c_cdir;
        new_temperature += (temperatures[tidy-1][tidx-1] + temperatures[tidy+1][tidx-1] +
                temperatures[tidy-1][tidx+1] + temperatures[tidy+1][tidx+1]) * rest_c_c * d_c_cdiag;
...
__host__ void cuda_do_compute(const struct parameters* p, struct results *r)
{
    ...
    err = cudaMemcpyToSymbol(&d_c_cdir, &c_cdir, sizeof(double), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) fprintf(stderr, "Error in cudaMemcpyToSymbol d_c_cdir: %s\n", cudaGetErrorString( err ));
    err = cudaMemcpyToSymbol(&d_c_cdiag, &c_cdiag, sizeof(double), cudaMemcpyHostToDevice);
    if (err != cudaSuccess) fprintf(stderr, "Error in cudaMemcpyToSymbol d_c_cdiag: %s\n", cudaGetErrorString( err ));
